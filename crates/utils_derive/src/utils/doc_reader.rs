...
pub(crate) fn extract_derive_doc_reader(input: &mut DeriveInput) -> TokenStream {
    ...
    let recursive_items = match &input.data {
        Data::Struct(data) => {
            data.fields.iter().enumerate().map(|(i, field)| {
                ...
                quote! {
                    {
                        let node = <#field_ty as utils::DocReaderInternal>::get_node();
                        let final_node = match node {
                            utils::DocNode::Leaf(_) => utils::DocNode::Leaf(#field_docs),
                            utils::DocNode::Node(_, children) => utils::DocNode::Node(#field_docs, children),
                        };
    ...
    let expanded = quote! {
        impl #impl_generics utils::DocReader for #struct_name #ty_generics #where_clause {
            ...
            fn get_recursive_docs() -> utils::DocNode {
                let mut map = indexmap::IndexMap::new();
                #(#recursive_items)*
                utils::DocNode::Node(#struct_docs, map)
            }
        }

        impl #impl_generics utils::DocReaderInternal for #struct_name #ty_generics #where_clause {
            fn get_node() -> utils::DocNode {
...
pub(crate) fn collect_fields_docs_list(fields: &Fields) -> Vec<proc_macro2::TokenStream> {
    fields
        .iter()
        .enumerate()
        .map(|(i, field)| {
